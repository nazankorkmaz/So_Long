...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/17 10:21:39 by nkorkmaz          #+#    #+#             */
/*   Updated: 2023/08/18 13:51:34 by nkorkmaz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_score(t_win *win)
{
	...
	mlx_string_put(win->mlx, win->win, 27, 33, 0x000000FF, score);
	free(score);
}
...
void	edit_map(int keycode, t_win *win, char **map)
{
	...
	else if (map[y][x] != '1' && (map[y][x] != 'E' || win->map->c_count == 0))
	{
		new_map(win, map, x, y);
